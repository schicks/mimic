...
if ($latest_version != $changelog_version) {
  echo "::debug::Changelog version ($changelog_version) does not match package version ($latest_version)"
  exit 1
}
if ($latest_version != $release_version) {
  echo "::debug::Release version ($release_version) does not match package version ($latest_version)"
  exit 1
}
if ($latest_version != $release_name) {
  echo "::debug::Release name ($release_name) does not match package version ($latest_version)"
  exit 1
}